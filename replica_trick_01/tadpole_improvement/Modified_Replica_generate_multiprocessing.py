...
For values of alpha [0 - 1] creates a set of replica lattices and analyses the 
Wilson action evolution across configurations and <S_L+1 - S_L> for each alpha.
...
n = 2
s = 1
u0 = 1.0  # placeholder; overwritten below if USE_TADPOLE
Nstart_analyze = 100 #update once thermalized
run_number = 1  #
...
def calibrate_u0():
    ...
    if os.path.exists(cache_file) and os.path.getsize(cache_file) > 0:
        with open(cache_file, "r") as f:
            cached_u0 = float(f.read().strip())
        trace(f"Loaded tadpole u0 from cache {cache_file}: {cached_u0}")
        return cached_u0

    calibrated_u0 = tadpole.solve_u0_selfconsistent(Nt, Nx, Ny, Nz, beta, n, s)
    with open(cache_file, "w") as f:
        f.write(f"{calibrated_u0}\n")
    trace(f"Calibrated tadpole u0 and wrote {cache_file}: {calibrated_u0}")
    return calibrated_u0
...
def create_base():
    ...
    if USE_TADPOLE:
        new_base_dir += "_tadpole"
    new_base_dir = os.path.join(base_dir, new_base_dir)
    return new_base_dir

def run_replica_simulation(new_base_dir, alpha, ensemble, u0_value):
    import os
    import gauge_latticeqcd as glqcd
    dir_name = new_base_dir + f"_alpha{alpha}_ensemble{ensemble}"
    filename_U1 = os.path.join(dir_name, f"config_{Nstart:01d}_U1.npy")
    if os.path.exists(filename_U1):
        U1 = np.load(filename_U1)
        print(f"Loading config {Nstart:01d}...")
    else:
        U1 = None
        print(f"Generating config {Nstart:01d}...")
    U = glqcd.ReplicaLattice(Nt, Nx, Ny, Nz, beta, u0_value, n, s, U1)
    matrices = glqcd.create_su3_set()
    U.markov_chain_sweep_replica(Ncfg, matrices, Nhits, dir_name, alpha, Nstart)

def main_control_run(new_base_dir, u0_value):
    # Creates and runs simulation
    processes = []
    for alpha in alphas:
        for ensemble in ensembles:
            p = Process(target=run_replica_simulation, args=(new_base_dir, alpha, ensemble, u0_value))
            p.start()
            processes.append(p)
...
def process_alpha_ensemble(new_base_dir, cfg_start, cfg_end, alpha, ensemble, u0_value):
    ...
    for cfg in range(cfg_end - cfg_start):
        ...
        lattice = glqcd.ReplicaLattice(Nt, Nx, Ny, Nz, beta, u0_value, n, s, U1=U1)
        S1 = lattice.calc_action_replica(xcutoff_L)
        S2 = lattice.calc_action_replica(xcutoff_Lp1)
        action_1.append(S1)
        action_2.append(S2)
        S_int = lattice.calc_S_int(alpha, xcutoff_L, xcutoff_Lp1)
        int_actions.append(S_int)
    return alpha, ensemble, action_1, action_2, int_actions

def plot_actions(new_base_dir, u0_value):
    jobs = []
    for alpha in alphas:
        for ensemble in ensembles:
            #jobs.append((new_base_dir, Nstart, Nend, alpha, ensemble))
            jobs.append((new_base_dir, 0, Nend, alpha, ensemble, u0_value))
    results = []
    with Pool() as pool:
...
def compute_action_diff(cfg_start, cfg_end, dir, alpha, ensemble, u0_value):
    import gauge_latticeqcd as glqcd
    dir_name = f"{dir}_alpha{alpha}_ensemble{ensemble}"
    trace(f"[diff] Begin alpha={alpha}, ensemble={ensemble}, cfg=[{cfg_start},{cfg_end})")

    # define the two cutoffs here (L and L+1 in the paper)
    xcutoff_L   = Nx - s # this is L
    xcutoff_Lp1 = Nx - (s + 1)      # this is L+1

    diffs = []
    for cfg in range(cfg_start, cfg_end):
        path = os.path.join(dir_name, f"config_{cfg}_U1.npy")
        print(f"[load] alpha={alpha}, e={ensemble}, cfg={cfg} -> {path}", flush=True)
        U1 = np.load(path)
        lattice1 = glqcd.ReplicaLattice(Nt, Nx, Ny, Nz, beta, u0_value, n, s, U1=U1)

        SL   = lattice1.calc_action_replica(xcutoff_L)      # S_L
        SLp1 = lattice1.calc_action_replica(xcutoff_Lp1)    # S_{L+1}

        d = (SLp1 - SL) / (Ny * Nz)
        diffs.append(d)
        print(f"[diff] alpha={alpha}, e={ensemble}, cfg={cfg}, dS={d}", flush=True)

    S_diffs = np.array(diffs)
    trace(f"[diff] Done alpha={alpha}, ensemble={ensemble}")
    return alpha, S_diffs

 
def analyse_action_differences(new_base_dir, u0_value):
    jobs = [(Nstart_analyze, Nend, new_base_dir, alpha, e, u0_value) for alpha in alphas for e in ensembles]
    ...
    plt.text(0.05, 0.1, f"Run number: {run_number}\n$N_t x N_s^3 = {Nt}x{Nx}^3$\nEnsembles: {ensembles[0]}-{ensembles[-1]}\nC({l:.2f}) fm = {c:.5f}\nu0 = {u0_value:.5f}", transform=plt.gca().transAxes, fontsize=10, bbox = dict(boxstyle='round', fc='blanchedalmond', ec='orange', alpha=0.5))
...
### M. Guagnelli et al. [ALPHA Collaboration], Nucl. Phys. B 535, 389 (1998); Eq. (18)
def sommer_scale():
    r0 = 0.5 #fm
    tmp = -1.6805 - 1.7139 * (beta - 6.) + 0.8155 * (beta - 6.)**2 - 0.6667 * (beta - 6.)**3
    return r0 * np.exp(tmp)
...
def main():
    ...
    main_control_run(new_base_dir, u0)
    #plot_actions(new_base_dir, u0)
    S_alpha_diffs = analyse_action_differences(new_base_dir, u0)
    c, l = c_func(S_alpha_diffs)
