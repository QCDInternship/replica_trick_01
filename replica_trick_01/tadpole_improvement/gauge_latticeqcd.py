...
def fn_eval_point_S(U, t, x, y, z, beta, u0 = 1.):
    ...
    for mu in range(1, 4):  #sum over \mu > \nu spacetime dimensions
        for nu in range(mu):
            tmp += ( 1. - np.real(np.trace( fn_plaquette(U, t, x, y, z, mu, nu) )) / 3. / u0**4 ) # tadpole-improved
    return beta * tmp
...
class lattice():
    ...
    def dS_staple(self, t, x, y, z, mu):
        ...
        return tmp1 / self.u0**3, tmp2 / self.u0**3 # tadpole-improved
    ...
    def dS_staple_rectangle(self, t, x, y, z, mu):
        ...
        ans_fwd = (5. * plaquette1 / self.u0**3 / 9.) - (rectangle_fwd / self.u0**5 / 36.) # tadpole-improved
        ans_bwd = (5. * plaquette2 / self.u0**3 / 9.) - (rectangle_bwd / self.u0**5 / 36.) # tadpole-improved
    ...
    def deltaS(self, link, updated_link, staple1, staple2):
        ...
        return (-self.beta / 3.0 / self.u0 ) * np.real(np.trace(tmp1 + tmp2)) # tadpole-improved
    ...
    def markov_chain_sweep(self, Ncfg, matrices, initial_cfg=0, save_name='', Nhits=10, action='W', Nu0_step=10, Nu0_avg=10): # tadpole-improved
        ratio_accept = 0.
        matrices_length = len(matrices)
        if save_name:
            output = os.path.join(save_name, 'link_' + os.path.basename(save_name) + '_')
...
class ReplicaLattice:
    ...
    def eval_point_S_replica(self, txyz, xcutoff):
        ...
        for mu in range(1, 4):  #sum over \mu > \nu spacetime dimensions # CHANGED TO INCLUDE TIMELIKE
            for nu in range(mu):
                tmp += ( 1. - np.real(np.trace( self.plaquette_replica(txyz, mu, nu, xcutoff) )) / 3. / self.u0**4 ) # tadpole-improved
        ...
        for t in range(self.Nt):
            for x in range(self.Nx):
                for y in range(self.Ny):
                    for z in range(self.Nz):
                        txyz = [t, x, y, z]
                        S += self.eval_point_S_replica(txyz, xcutoff) # tadpole-improved
        return S
    ...
    def calc_S_int(self, alpha, xcutoff_1, xcutoff_2):
        ...
        S_int = (1. - alpha) * S_1 + alpha * S_2 # tadpole-improved
        return S_int
    ...
    def deltaS(self, link, updated_link, staple1, staple2):
        tmp1 = np.dot( (updated_link - link), staple1)
        tmp2 = np.conj(np.dot( (updated_link - link), staple2)).T
        return (-self.beta / 3. / self.u0 ) * np.real(np.trace(tmp1 + tmp2)) # tadpole-improved
                            ...
                            for z in range(self.Nz):
                                for mu in range(4):
                                    ...
                                    for _ in range(Nhits):
                                        ...
                                        dS_int = self.delta_S_int(alpha, dS_1, dS_2) # tadpole-improved
