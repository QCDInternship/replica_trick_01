...
def load_u0_cache():
    ...
    raise FileNotFoundError(
        f"missing tadpole cache {u0_cache_file}; run generation first so "
        "analysis uses the same u0 as generation"
    )
...
def c_func(S_alpha_diffs):
    ...
    print(f"l = {l} fm")
    V = (a**3) * Nx * Ny * Nz
    # Convention: C is dimensionless in lattice units; for a fm^3 quantity,
    # carry the explicit a**3 conversion consistently through this ratio.
